...
def month_limited(func):
    ...
    @wraps(func)
    def month_wrapper(_, report_name, args):

        start = pd.Timestamp(args['FromSettlementDate'])
        end = pd.Timestamp(args['ToSettlementDate'])
        ...
        for _start, _end in blocks:
            try:
                _temp_args = {
                    'FromSettlementDate': _start.strftime('%Y-%m-%d'),
                    'ToSettlementDate': _end.strftime('%Y-%m-%d')
                }

                frame = func(_, report_name, _temp_args)
        ...
        df = pd.concat(frames)
        # df = df.loc[~df.duplicated(subset=['settlementDate','settlementPeriod'], keep='first')]
        return df
...
class ElexonPandasClient(ElexonRawClient):
    ...
    def __call__(self, report_name: str = None, args: dict = None):
        ...
        response = self.base_request(report_name, args)
        parsed_response = self._parse_response(response, report_name)
